...
| `routes.py` | REST API endpoints (GET-only JSON). Serves trades, stats, risk, health, scanner status, balance, scan cycles, and recent signals. All trade/stats queries delegate to `TradeJournal` methods -- no raw `aiosqlite` usage. |
