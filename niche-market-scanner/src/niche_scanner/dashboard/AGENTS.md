...
| `pages.py` | HTML page route handlers. Renders Jinja2 templates for `/`, `/trades`, `/signals`, `/config`. Supports full-page and HTMX partial rendering. The `/` route loads the last 200 `BalanceTracker` snapshots and embeds them as JSON in the page so Chart.js can render the balance history client-side. |
